...
class MirrorLeechListener:
        ...
        if self.isLeech:
            ...
            if not files:
                ...
                if self.isSuperGroup:
                    await sendMessage(self.message, f'{msg}<blockquote>Files has been sent to your inbox</blockquote>', iButton.build_menu(1))
                else:
                    await deleteMessage(self.botpmmsg)
            if self.seed:
                if self.newDir:
            ...
            if link:
                ...
                buttons = extra_btns(buttons)
                button = buttons.build_menu(2)
            msg += f'<blockquote>• Uploaded by: </blockquote>{self.tag}\n'
            msg += f'<blockquote>• User ID: </blockquote><code>{self.message.from_user.id}</code>\n\n'
    ...
    async def onUploadError(self, error):
        ...
        msg += f'<blockquote>Reason:</blockquote> {escape(error)}\n'
        msg += f'<blockquote>Elapsed:</blockquote> {get_readable_time(time() - self.message.date.timestamp())}'
